	...
	onLoad: function(options) {
		...
		wx.getSystemInfo({
			success: function(res) {
				...
				var calc = clientHeight * rpxR - 110;
				that.setData({
					winHeight: calc
				});
			}
	...
	generatePosters() {
		...
		wx.getImageInfo({
			...
			success(res) {
				...
				ctx.drawImage(_self.data.posters.head_img, 0, 0, wx.getSystemInfoSync().screenWidth * 0.8, wx.getSystemInfoSync().screenWidth * 0.8);
				ctx.setFontSize(24)
				ctx.setFillStyle('#FF9900')
				ctx.setTextAlign('left')
				ctx.fillText(`￥` + _self.data.goods.retail_price, 0, wx.getSystemInfoSync().screenWidth * 0.8 + 60)
				ctx.setFontSize(14)
				ctx.setFillStyle('#000')
				ctx.fillText(_self.data.goods.name, 0, wx.getSystemInfoSync().screenWidth * 0.8 + 20);
				ctx.setFontSize(12)
				ctx.setFillStyle('#888')
				ctx.fillText('长按识别小程序码', 0, wx.getSystemInfoSync().screenWidth * 0.8 + 90);
				ctx.drawImage('../../static/images/qrcode.png', wx.getSystemInfoSync().screenWidth * 0.5, wx.getSystemInfoSync().screenWidth * 0.8 + 30, wx.getSystemInfoSync().screenWidth * 0.2, wx.getSystemInfoSync().screenWidth * 0.2);
				ctx.draw();
				/*
  			setTimeout(function() {
