...
Page({
  ...
  getAddressDetail() {
    let that = this;
    util.request(api.AddressDetail, { id: that.data.addressId },'POST','application/x-www-form-urlencoded').then(function (res) {
      if (res.errno === 0) {
        if(res.data){
            that.setData({
                address: res.data,
                isDefaultbtn:res.data.is_default==1?'false':'true'
            });
        }
      }
    });
  ...
  chooseRegion() {
    ...
    if (address.province_id > 0 && address.city_id > 0 && address.district_id > 0) {
      ...
      this.setData({
        selectRegionList: [
          { id: 0, name: '省份', parent_id: 1, type: 1 },
          { id: 0, name: '请选择', parent_id: 1, type: 2 },
          { id: 0, name: '请选择', parent_id: 1, type: 3 }
        ],
        regionType: 1
      })
  ...
  getRegionList(regionId) {
    ...
    util.request(api.RegionList, { parentId: regionId },'POST','application/x-www-form-urlencoded').then(function (res) {
      if (res.errno === 0) {
        that.setData({
  ...
  saveAddress(){
    ...
    if (!(/^1[345789]\d{9}$/.test(address.telNumber))) {
    ...
    util.request(api.AddressSave, {
      ...
      is_default: this.data.isDefaultbtn!=true?1:address.is_default,
